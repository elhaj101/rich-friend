...
const STATUS_STYLE: Record<AppointmentStatus, { bg: string; color: string }> = {
  ...
  confirmed: { bg: "#F3DDD0", color: "#8A3D20" },
...
export default function AppointmentCard({ appt }: { appt: Appointment }) {
  ...
  return (
    <div className="bg-white border border-card-line rounded-[10px] p-5 md:p-6 flex gap-4">
      <div className="shrink-0 w-11 h-11 rounded-full flex items-center justify-center text-terracotta bg-terracotta-tint">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <h3 className="font-serif-dash text-charcoal" style={{ fontSize: 19, lineHeight: 1.2 }}>
        ...
        {!isPast && (
          <a
            ...
            className="inline-flex items-center gap-2 mt-4 font-sans text-[12px] font-semibold tracking-[0.04em] text-charcoal no-underline hover:text-terracotta transition-colors"
          >
            {t.dashApptJoin} →
          </a>
        )}
